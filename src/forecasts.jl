...
function feature_indexes_sgns(asset_rtn_sgn, kst, klt,kmom, ϱ = 2)
    ...
    sig_1 = (1/kst)*sum(asset_rtn_sgn[t-d] for d=0:kst-1)
    sig_2 = (1/klt)*sum(asset_rtn_sgn[t-d] for d=0:klt-1)

    # sig_3 and sig_4 are exponential moving average (short and long)
    sig_3 = (1.0/sum(exp(-d/kst) for d=1:kst))*sum(exp(-d/kst)*asset_rtn_sgn[t-d] for d=0:kst-1)
    sig_4 = (1.0/sum(exp(-d/klt) for d=1:klt))*sum(exp(-d/klt)*asset_rtn_sgn[t-d] for d=0:klt-1)
    # RSI
    R_pos = 0
    R_neg = 0
    num_pos = 0
    ...
    σ_st_6 = sqrt((1.0/kst)*sum((asset_rtn_sgn[t-d]-sig_1)^2 for d=0:kst-1))
    σ_lt_7 = sqrt((1.0/klt)*sum((asset_rtn_sgn[t-d]-sig_2)^2 for d=0:klt-1))
    # sig_6 and sig_7 are
    ...
    σ_st_8 = sqrt((1.0/kst)*sum((asset_rtn_sgn[t-d]-sig_3)^2 for d=0:kst-1))
    σ_lt_9 = sqrt((1.0/klt)*sum((asset_rtn_sgn[t-d]-sig_4)^2 for d=0:klt-1))
    if sig_3 <= 0
...
function predict_return(asset_rtn_sgn,num_t, kst_a, klt_a,kmom, ϱ = 2)
    ...
    for i = 1:num_train
      ...
      kst = Int64(floor(klt/2.))
      sig_features[num_train+1-i,:] = feature_indexes_sgns(asset_rtn_sgn[1:numD-i], kst, klt ,kmom)
    end
    # optimize weights (least squares - pseudo inverse)
    try
      weights_opt = \(sig_features'sig_features,sig_features'asset_rtn_sgn[numD-num_train+1:numD])
    catch
